...
It should not modify `.agents/skills/`, prompts, MCP config, demo workspace, or user business code. Existing deployment files are skipped unless `--overwrite` is explicitly supplied. Generated agent YAML stays under `.azd/pi-foundry/generated/`; `.azd/pi-foundry/azd-agent.mjs` passes `AGENT_DEFINITION_PATH` to azd package/deploy so root `agent.yaml` is not persistently required.
