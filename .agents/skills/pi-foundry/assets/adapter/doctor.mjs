...
function commandResult(command, args = [], options = {}) {
  try {
    return { ok: true, stdout: execFileSync(command, args, { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], timeout: options.timeout ?? 120000 }).trim() };
  } catch (error) {
...
async function main() {
  ...
  if (azureYaml) {
    ...
    if (/workflows:\s*[\s\S]*up:\s*[\s\S]*azd-agent\.mjs\s+package --all[\s\S]*azd-agent\.mjs\s+deploy --all/.test(azureYaml)) pass("azure.yaml wraps azd package/deploy with pi-foundry agent definition path");
    else warn("azure.yaml does not appear to wrap azd package/deploy with .azd/pi-foundry/azd-agent.mjs");
  ...
  if (rootAgentYaml?.includes("Generated by pi-foundry")) warn("legacy generated root agent.yaml exists; pi-foundry now keeps agent YAML under .azd/pi-foundry/generated/");
  else if (rootAgentYaml) warn("root agent.yaml exists; pi-foundry deploy uses .azd/pi-foundry/generated/agent.yaml via AGENT_DEFINITION_PATH");
  if (rootManifestYaml?.includes("Generated by pi-foundry")) warn("legacy generated root agent.manifest.yaml exists; pi-foundry now keeps agent YAML under .azd/pi-foundry/generated/");
  else if (rootManifestYaml) warn("root agent.manifest.yaml exists; pi-foundry deploy uses .azd/pi-foundry/generated/agent.manifest.yaml as source material");
